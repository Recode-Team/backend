...
// create
router.post('/:group', verifyToken, upload.single('audioFile'), async (req, res) => {
  ...
  const bucketName = 'minutes-bucket';
  ...
  fileStream.on('finish', async () => {
    ...
    const summaryResponse = await fetch(`https://api.openai.com/v1/engines/${engine}/completions`, {
      method: 'POST',
      headers: {
    ...
    const titleResponse = await fetch(`https://api.openai.com/v1/engines/${engine}/completions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${gptApiKey}`,
    ...
    const dresult = await minutes.create(
      {transcription : speechTranscription, summary : summary, title: title})
    .catch(err => {
        console.error(err)
    })
    ...
    if (dresult == undefined){
      ...
      res.send({"result" : {"transcription" : speechTranscription, "summary" : summary, "title": title}});
    }
  });
